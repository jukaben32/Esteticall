...
export async function POST(request: Request, props: { params: Promise<{ agentId: string }> }) {
  ...
  const [allServices, assignedServiceIds, packagesResult, knowledgeDocs, platformKnowledgeDocs] = await Promise.all([
    listServicesForBusiness(supabase, business.id),
    listServiceIdsForAgent(supabase, agent.id),
    supabase.from('packages').select('*').eq('business_id', business.id).eq('is_active', true),
