...
import { markConsentFormSigned } from '@/services/consentForms'
...
export async function PATCH(request: Request, props: { params: Promise<{ consentFormId: string }> }) {
  ...
  const consentForm = await markConsentFormSigned(ctx.supabase, ctx.business.id, params.consentFormId, parsed.data.signatureName)
