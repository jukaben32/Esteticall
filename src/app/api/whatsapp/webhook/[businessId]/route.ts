...
export async function POST(request: Request, props: { params: Promise<{ businessId: string }> }) {
  ...
  const [allServices, assignedServiceIds, packagesResult, knowledgeDocs, platformKnowledgeDocs, historyRes] = await Promise.all([
    listServicesForBusiness(supabase, businessId),
    listServiceIdsForAgent(supabase, agent.id),
    supabase.from('packages').select('*').eq('business_id', businessId).eq('is_active', true),
